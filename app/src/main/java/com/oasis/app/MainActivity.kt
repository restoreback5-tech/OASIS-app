...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    private fun extractContactName(cmd: String): String {
        val keywords = listOf("llamar", "llama", "mensaje", "mensajes", "whatsapp", "wasap", "contacto", "contactos", "agenda", "buscar", "a", "al", "la", "los", "las")
        val words = cmd.split(" ")
        val nameWords = words.filter { word -> !keywords.contains(word) && word.length > 2 }
        return nameWords.joinToString(" ").capitalize()
    }
    ...
    private fun showAppsMenuDialog() {
        ...
        dialogView.findViewById<Button>(R.id.app_calculator).setOnClickListener { try { startActivity(Intent("android.intent.action.CALCULATOR")) } catch (e: Exception) { speak("Calculadora no disponible") }; dialog.dismiss() }
        dialogView.findViewById<Button>(R.id.app_camera).setOnClickListener { try { startActivity(Intent("android.media.action.IMAGE_CAPTURE")) } catch (e: Exception) { speak("Cámara no disponible") }; dialog.dismiss() }
        dialogView.findViewById<Button>(R.id.app_files).setOnClickListener { try { startActivity(Intent("android.intent.action.OPEN_DOCUMENT_TREE")) } catch (e: Exception) { speak("Archivos no disponible") }; dialog.dismiss() }
        dialogView.findViewById<Button>(R.id.app_settings).setOnClickListener { try { startActivity(Intent("android.settings.SETTINGS")) } catch (e: Exception) { speak("Ajustes no disponible") }; dialog.dismiss() }
