...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    override fun onInit(status: Int) {
        if (status == TextToSpeech.SUCCESS) {
            ...
            handler.postDelayed({ speak("Bienvenido a OASIS") }, 500)
        }
    }
        ...
        try {
            ...
            speechRecognizer?.setRecognitionListener(object : RecognitionListener {
                ...
                override fun onBeginningOfSpeech() {
                    speechBubble.text = "👂 Te escucho..."
                }
                override fun onRmsChanged(rmsdB: Float) {}
                override fun onBufferReceived(buffer: ByteArray?) {}
                override fun onEndOfSpeech() {}
                override fun onError(error: Int) {
                    speechBubble.text = "Bienvenido a OASIS"                    listenRunnable = Runnable { startListening() }
                    handler.postDelayed(listenRunnable!!, 2000)
                }
                override fun onResults(results: Bundle?) {
                    ...
                    listenRunnable = Runnable { startListening() }
    ...
    private fun extractContactName(cmd: String): String {
        val keywords = listOf("llamar", "llama", "mensaje", "mensajes", 
            "whatsapp", "wasap", "contacto", "contactos", "agenda", 
            "buscar", "a", "al", "la", "los", "las")
        val words = cmd.split(" ")
    ...
    private fun searchContacts(contactName: String) {        try {
    ...
    private fun showAppsMenuDialog() {
        try {
            ...
            dialogView.findViewById<Button>(R.id.app_calculator).setOnClickListener {
                try {
                    startActivity(Intent("android.intent.action.CALCULATOR"))
                } catch (e: Exception) {
                    speak("Calculadora no disponible")
                }
                dialog.dismiss()
            }
            ...
            dialogView.findViewById<Button>(R.id.app_camera).setOnClickListener {
                try {
                    startActivity(Intent("android.media.action.IMAGE_CAPTURE"))                } catch (e: Exception) {
            ...
            dialogView.findViewById<Button>(R.id.app_settings).setOnClickListener {
                try {
                    ...
                    speak("Ajustes no disponible")
                }
                dialog.dismiss()
            }

            dialogView.findViewById<Button>(R.id.app_flashlight).setOnClickListener {
                try { toggleFlashlight() } catch (e: Exception) { speak("Linterna no disponible") }
                dialog.dismiss()
            }
            ...
            dialogView.findViewById<Button>(R.id.app_calendar).setOnClickListener {
                try {
                    ...
                    speak("Calendario no disponible")
                }
                dialog.dismiss()
            }
    ...
    private fun toggleFlashlight() {
        try {
            ...
            speak("Linterna encendida")
            handler.postDelayed({
                try {
                    cameraManager.setTorchMode(cameraId, false)
                    speak("Linterna apagada")
                } catch (e: Exception) {}
            }, 30000)
        } catch (e: Exception) {
            speak("Linterna no disponible")
        }
    }
}
