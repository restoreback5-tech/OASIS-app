...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.btn_more).setOnClickListener {            playSound(R.raw.touch)
            showAppsMenuDialog()
        }
    }
    ...
    private fun startListening() {
        ...
        speechRecognizer?.setRecognitionListener(object : RecognitionListener {
            ...
            override fun onResults(results: Bundle?) {
                ...
                if (!matches.isNullOrEmpty()) {                    val cmd = matches[0].lowercase(Locale.getDefault())
                    processCommand(cmd)
                }
                listenRunnable = Runnable { startListening() }
    ...
    private fun processCommand(cmd: String) {
        when {
            ...
            cmd.contains("contacto") || cmd.contains("agenda") || cmd.contains("buscar") -> {
                val contactName = extractContactName(cmd)                if (contactName.isNotEmpty()) {
    ...
    private fun extractContactName(cmd: String): String {
        ...
        return nameWords.joinToString(" ").replaceFirstChar {
