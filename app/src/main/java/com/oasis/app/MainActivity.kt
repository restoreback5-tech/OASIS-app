...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fun updateTime() {
            ...
            tvCurrentTime.text = String.format("%02d:%02d",                c.get(Calendar.HOUR_OF_DAY),
        ...
        findViewById<Button>(R.id.btn_more).setOnClickListener {
            try {
                playSound(R.raw.volumeincremental)
                showAppsMenuDialog()
            } catch (e: Exception) {                speak("Error al abrir menú")
            }
        }
    }
        ...
        try {
            ...
            speechRecognizer?.setRecognitionListener(object : RecognitionListener {
                ...
                override fun onResults(results: Bundle?) {
                    val matches = results?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)                    if (!matches.isNullOrEmpty()) {
        ...
        try {
            when {
                ...
                cmd.contains("contacto") || cmd.contains("agenda") || cmd.contains("buscar") -> {
                    ...
                    } else {                        playSound(R.raw.volumeincremental)
                        speak("Abriendo contactos")
                        startActivity(Intent(Intent.ACTION_VIEW, ContactsContract.Contacts.CONTENT_URI))
                    }
                }
    ...
    private fun showAppsMenuDialog() {
        try {
            ...
            dialogView.findViewById<Button>(R.id.app_flashlight).setOnClickListener {
                speak("Activa la linterna desde el panel de notificaciones")
                dialog.dismiss()
            }
